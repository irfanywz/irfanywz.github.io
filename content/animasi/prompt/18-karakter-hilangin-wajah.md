---
title: "Hapus Wajah Karakter"
slug: "remove-wajah-karakter"
description: "Prompt builder untuk menghapus fitur wajah tertentu (mata, alis, hidung, atau mulut) secara bersih dan natural untuk keperluan animasi 2D"
icon: "icon-[ri--user-unfollow-line]"
categories:
  - "Karakter"
---

<div class="max-w-4xl mx-auto mt-6 space-y-6" x-data="removeFaceApp()" x-init="initSwiper()">
...
<i class="icon-[ri--user-unfollow-line] text-indigo-600 dark:text-indigo-400"></i> Remove Face Prompt Builder
</h3>
<div class="flex items-center gap-2">
<!-- Tombol View Base Prompt Remove Face -->
...
<!-- Input Target Wajah & Active Image Preview -->
...
<label class="text-xs font-medium text-gray-600 dark:text-gray-400">[FACE AREAS TO REMOVE]</label>
<div class="relative flex items-center">
<input type="text" x-model="faceAreas" @focus="$el.select()" placeholder="e.g. Completely remove eyes and eyebrows, leaving the mouth and nose intact..." class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl pl-3 pr-10 py-2 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:border-indigo-500">
<button @click="showModal = true; $nextTick(() => { if(categorySwiper) categorySwiper.update(); })" class="absolute right-2 text-indigo-600 dark:text-indigo-400 p-1 hover:bg-indigo-50 dark:hover:bg-indigo-950 rounded-lg cursor-pointer" title="Pilih Template Hapus Wajah">
...
<!-- Modal Picker Database Template Hapus Wajah -->
...
<h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200">Pilih Template Penghapusan Wajah</h3>
...
<template x-for="(cat, catName) in removeFaceDatabase" :key="catName">
<div class="swiper-slide !w-auto">
...
<script>
function removeFaceApp() {
    return {
        faceAreas: 'Completely remove eyes and eyebrows, leaving a blank natural skin surface',
        activeImage: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%231e1b4b"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23c7d2fe" font-size="12" font-family="sans-serif">Blank Face</text></svg>',
        copied: false,
        descCopied: false,
        showModal: false,
        ...
        removeFaceDatabase: {
            "Penghapusan Fitur": [
                {
                    title: "Hapus Mata & Alis",
                    description: "Completely remove eyes and eyebrows, leaving a blank natural skin surface",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%231e3a8a"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23bfdbfe" font-size="10" font-family="sans-serif">No Eyes</text></svg>'
                },
                {
                    title: "Hapus Seluruh Wajah",
                    description: "Completely remove eyes, eyebrows, nose, and mouth, leaving the entire face clean and blank",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23065f46"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23a7f3d0" font-size="10" font-family="sans-serif">Blank Face</text></svg>'
                },
                {
                    title: "Hapus Mulut Saja",
                    description: "Completely remove the mouth and lip lines, keeping eyes and nose intact",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23581c87"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23e9d5ff" font-size="10" font-family="sans-serif">No Mouth</text></svg>'
                }
            ]
        },
        ...
        get filteredDatabase() {
            if (this.activeCategory === 'All') {
                let allItems = [];
                for (let cat in this.removeFaceDatabase) {
                    allItems.push(...this.removeFaceDatabase[cat]);
                }
                return allItems;
            }
            return this.removeFaceDatabase[this.activeCategory] || [];
        },
        ...
        selectItem(item) {
            this.faceAreas = item.description;
        ...
        randomize() {
            ...
            this.faceAreas = rand.description;
            this.activeImage = rand.image;
        },

        get fullPrompt() {
            let areas = this.faceAreas ? this.faceAreas.trim() : 'Completely remove specified facial features and reconstruct the face naturally.';
