---
title: "Redesain Objek"
slug: "redesain-objek"
description: "Prompt builder untuk melakukan redesain objek, prop, atau item berdasarkan referensi gambar dengan siluet, proporsi, detail, dan gaya baru yang unik"
icon: "icon-[ri--magic-line]"
categories:
  - "Objects"
---

<div class="max-w-4xl mx-auto mt-6 space-y-6" x-data="redesignObjectApp()" x-init="initSwiper()">
...
<i class="icon-[ri--magic-line] text-indigo-600 dark:text-indigo-400"></i> Object Redesign Prompt Builder
</h3>
<div class="flex items-center gap-2">
<!-- Tombol View Base Prompt Redesign Object -->
...
<!-- Input Design Direction & Active Preview -->
...
<label class="text-xs font-medium text-gray-600 dark:text-gray-400">[CHANGE / DESIGN DIRECTION]</label>
<div class="relative flex items-center">
<input type="text" x-model="designDirection" @focus="$el.select()" placeholder="e.g. Redesign into a futuristic cyberpunk tech version with glowing neon accents and metallic plating..." class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl pl-3 pr-10 py-2 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:border-indigo-500">
<button @click="showModal = true; $nextTick(() => { if(categorySwiper) categorySwiper.update(); })" class="absolute right-2 text-indigo-600 dark:text-indigo-400 p-1 hover:bg-indigo-50 dark:hover:bg-indigo-950 rounded-lg cursor-pointer" title="Select Redesign Template">
...
<!-- Modal Picker Database Template Redesign -->
...
<h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200">Select Redesign Direction Template</h3>
...
<template x-for="(cat, catName) in redesignDatabase" :key="catName">
<div class="swiper-slide !w-auto">
...
<script>
function redesignObjectApp() {
    return {
        designDirection: 'Redesign into a futuristic cyberpunk tech version with glowing neon accents and metallic plating.',
        activeImage: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%231e1b4b"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23c7d2fe" font-size="10" font-family="sans-serif">Object Ref</text></svg>',
        copied: false,
        descCopied: false,
        showModal: false,
        ...
        redesignDatabase: {
            "Cyberpunk & Sci-Fi": [
                {
                    title: "Cyberpunk Tech Redesign",
                    description: "Redesign into a futuristic cyberpunk tech version with glowing neon accents and metallic plating",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%231e3a8a"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23bfdbfe" font-size="10" font-family="sans-serif">Cyberpunk</text></svg>'
                },
                {
                    title: "Sci-Fi Space Utility",
                    description: "Redesign into an advanced interstellar sci-fi equipment with holographic displays and composite carbon-fiber housing",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23312e81"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23c7d2fe" font-size="10" font-family="sans-serif">Sci-Fi</text></svg>'
                }
            ],
            "Fantasy & Steampunk": [
                {
                    title: "Steampunk Brass & Gear",
                    description: "Redesign into a Victorian steampunk mechanical version featuring polished brass, copper pipes, and exposed clockwork gears",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23b45309"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23fef3c7" font-size="10" font-family="sans-serif">Steampunk</text></svg>'
                },
                {
                    title: "Magical Fantasy Artifact",
                    description: "Redesign into an ancient magical fantasy artifact with carved runes, mystical crystals, and weathered dark wood",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23581c87"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23e9d5ff" font-size="10" font-family="sans-serif">Fantasy</text></svg>'
                }
            ],
            "Modern & Minimalist": [
                {
                    title: "Minimalist Modern Aesthetic",
                    description: "Redesign into a clean modern minimalist version with smooth matte surfaces, geometric proportions, and neutral monochrome color palette",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%23065f46"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23a7f3d0" font-size="10" font-family="sans-serif">Minimalist</text></svg>'
                },
                {
                    title: "Retro 80s Arcade Pop",
                    description: "Redesign into a vibrant retro 80s arcade style with pastel gradients, chunky rounded shapes, and playful colorful buttons",
                    image: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120"><rect width="120" height="120" fill="%237f1d1d"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23fecaca" font-size="10" font-family="sans-serif">Retro 80s</text></svg>'
                }
            ]
        },
        ...
        get filteredDatabase() {
            if (this.activeCategory === 'All') {
                let allItems = [];
                for (let cat in this.redesignDatabase) {
                    allItems.push(...this.redesignDatabase[cat]);
                }
                return allItems;
            }
            return this.redesignDatabase[this.activeCategory] || [];
        },
        ...
        selectItem(item) {
            this.designDirection = item.description;
        ...
        randomize() {
            ...
            this.designDirection = rand.description;
            this.activeImage = rand.image;
        },

        get fullPrompt() {
            let direction = this.designDirection ? this.designDirection.trim() : 'Redesign the object into a fresh, unique visual version.';

            return `Use the attached image as the OBJECT REFERENCE.
Redesign the object into a new and unique version based on this instruction:

[CHANGE / DESIGN DIRECTION]
[${direction}]

Apply the requested changes while keeping the object clearly recognizable as the same type of object.
Create a fresh design with a different silhouette, proportions, shapes, details, materials, or colors as needed by the instruction.
Do not simply copy the original design. Use the reference only to understand the object's identity and function.

RESULT
A clean, original, visually distinct redesign of the object based on [CHANGE / DESIGN DIRECTION].`;
        },
